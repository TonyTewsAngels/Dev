...
/**
 * This class encapsulates the functionality for rendering a lesson to the
 * screen.
 * 
...
public class Renderer {
	...
	public void renderPage(Page page) {
		...
		clearPage();
		/* Add the background */
		Rectangle bg = new Rectangle(bounds.getMaxX(), bounds.getMaxY(),
				Util.colorFromString(page.getPageColour()));
	...
	private void renderText(TextObject text) {
		...
		for (int i = 0; i < text.textFragments.size(); i++) {
			...
			if (richText.isBold())
				attrs.add(TextAttribute.BOLD);
			if (richText.isItalic())
				attrs.add(TextAttribute.ITALIC);
			if (richText.isUnderline())
				attrs.add(TextAttribute.UNDERLINE);
			if (richText.isSubscript())
				attrs.add(TextAttribute.SUBSCRIPT);
			if (richText.isSuperscript())
				attrs.add(TextAttribute.SUPERSCRIPT);
			if (richText.isStrikethrough())
				attrs.add(TextAttribute.STRIKETHROUGH);
			/* Add the rich text fragment to the buffer */
			textHandler.addStringToBuffer(richText.getText(),
					richText.getFont(), richText.getFontSize(),
					richText.getColor(), "#00000000",
	...
	private void renderVideo(VideoObject video) {
		videoHandler.createVideo((float) bounds.getMaxX() * video.getXStart(),
				(float) bounds.getMaxY() * video.getYStart(), 800,
				video.getSourcefile(), false, false);
	}

	/** Render an audio object on a page */
	private void renderAudio(AudioObject audio) {
		audioHandler.createAudio(
				(double) (bounds.getMaxX() * audio.getXStart()),
				(double) (bounds.getMaxY() * audio.getYStart()), 300, new File(
						audio.getSourcefile()), false, audio.isViewProgress(),
				false);
	}

	/** Render an image on a page */
	private void renderImage(ImageObject image) {
		imageHandler.insertImage(image.getSourcefile(), bounds.getMaxX()
				* image.getXStart(), bounds.getMaxY() * image.getYStart(),
				image.getxScaleFactor(), image.getyScaleFactor(),
				image.getRotation());
	}
	...
	private void renderGraphic(GraphicObject graphic) {
		...
		case OVAL:
			graphicsHandler.drawOval(
					(float) (bounds.getMaxX() * graphic.getXStart()),
					...
					graphic.getOutlineThickness(), Shadow.NONE,
					graphic.getRotation(), shading,
		...
		case RECTANGLE:
			graphicsHandler.drawRectangle(
					...
					(float) (bounds.getMaxY() * graphic.getYStart()),
					(float) (bounds.getMaxX() * graphic.getXEnd()),
					(float) (bounds.getMaxY() * graphic.getYEnd()), 0.0f, 0.0f,
					Util.colorFromString(graphic.getGraphicColour()),
					graphic.isSolid(),
					Util.colorFromString(graphic.getLineColor()),
					graphic.getOutlineThickness(), Shadow.NONE,
			...
			break;
		case LINE:
			graphicsHandler.drawLine(
					(float) (bounds.getMaxX() * graphic.getXStart()),
					(float) (bounds.getMaxY() * graphic.getYStart()),
					(float) (bounds.getMaxX() * graphic.getXEnd()),
					(float) (bounds.getMaxY() * graphic.getYEnd()),
					Util.colorFromString(graphic.getGraphicColour()),
					graphic.getOutlineThickness(), shading,
					Util.colorFromString(graphic.getShadingColor()));
			break;
		case TRIANGLE:
			System.out
					.println("Cannot Currently Draw Equi Triangle - Graphics Handler Unfinished");
			graphicsHandler.drawEquiTriangle();
			break;
		case ROUNDEDRECTANGLE:
			System.out
					.println("Cannot Currently Draw Rounded Rects - Graphics Handler Unfinished");
			graphicsHandler.drawRectangle(
					(float) (bounds.getMaxX() * graphic.getXStart()),
					(float) (bounds.getMaxY() * graphic.getYStart()),
					(float) (bounds.getMaxX() * graphic.getXEnd()),
					(float) (bounds.getMaxY() * graphic.getYEnd()), 150.0f,
					150.0f, Util.colorFromString(graphic.getGraphicColour()),
					graphic.isSolid(),
					Util.colorFromString(graphic.getLineColor()),
					graphic.getOutlineThickness(), Shadow.NONE,
					graphic.getRotation(), shading,
	...
	public void renderAnswerBox(AnswerBoxObject aBox) {
		answerBoxHandler.createAnswerBox(
				(float) (bounds.getMaxX() * aBox.getXStart()),
				(float) (bounds.getMaxY() * aBox.getYStart()),
				aBox.getCharacterLimit(), aBox.isRetry(),
				aBox.getCorrectAnswers(), aBox.getMarks(), aBox.isNumerical());
	}

	/** Add a multiple choice object to the screen */
	public void renderMultipleChoice(MultipleChoiceObject mChoice) {
		multipleChoiceHandler.createMultipleChoice(
				(float) (bounds.getMaxX() * mChoice.getXStart()),
				(float) (bounds.getMaxY() * mChoice.getYStart()),
				mChoice.getAnswers(), mChoice.getMultiChoiceType(),
				mChoice.getOrientation(), mChoice.isRetry());
	}
}